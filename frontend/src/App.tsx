...
function App() {
  ...
  useEffect(() => { setError(false); getDashboardOverview({ region: region || undefined, category: category || undefined, customer: customer || undefined }).then(setOverview).catch(() => setError(true)); }, [region, category, customer]);

  if (error) return <main className="loading">Dashboard data could not be loaded. Confirm the API is running on port 8000.</main>;
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      <section className="filter-bar" aria-label="Dashboard filters"><label>Customer<select value={customer} onChange={(event) => setCustomer(event.target.value)}><option value="">All customers</option>{filters.customers.map((item) => <option key={item}>{item}</option>)}</select></label><label>Region<select value={region} onChange={(event) => setRegion(event.target.value)}><option value="">All regions</option>{filters.regions.map((item) => <option key={item}>{item}</option>)}</select></label><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}><option value="">All categories</option>{filters.categories.map((item) => <option key={item}>{item}</option>)}</select></label><button onClick={() => { setRegion(""); setCategory(""); setCustomer(""); }} disabled={!region && !category && !customer}>Reset filters</button></section>
